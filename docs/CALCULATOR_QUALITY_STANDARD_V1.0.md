...
### □ 13. 원칙·예외 일관성 (권리·의무 안내형 계산기)
- [ ] 권리·의무 안내는 반드시 **원칙 → 예외** 순서로 서술되는가
  - 원칙: "~해야 한다 / ~받을 수 있다"를 먼저 명시
  - 예외: "단, ~한 경우에만 면제/제한된다"를 이후에 명시
...
## 연차수당 계산기 검증 결과 (2026-07-19, Phase 1·2 완료) ✅ Verified

| 항목 | 결과 |
|------|------|
| 계산식 | ✅ `daily_wage × unused_days` — 근로기준법 제60조제5항과 일치 |
| 법령 | ✅ AL-3/4 해결: faq[1] 원칙-예외 구조 확립. 제60조제5항·제36조(의무) + 제61조(촉진제도 요건 상세) |
| 경계값 | ✅ AL-1 해결(Phase 1): `daily_wage ≤ 0 or unused_days ≤ 0 → return null` |
| 예외 처리 | ✅ AL-1 해결: 음수/0 입력 → null (기존 패턴 재사용) |
| UI | ✅ AL-5 해결(Phase 2): notices — 법정 상한(25일) 초과 경고. AL-6 해결: `_formula` — "통상임금(일급) N원 × M일 = Y원" |
| 콘텐츠 정합성 | ✅ AL-2 해결(Phase 1): 통상임금 정의 교정. AL-7 해결(Phase 2): "일급" → "통상임금(일급)" 전위치 병기 |
| 원칙-예외 일관성 | ✅ C-13 적용: FAQ / article HTML FAQ / article 주의사항 본문 3곳 동일 구조 확인 |
| FAQ | ✅ AL-4 해결(Phase 2): 제61조 촉진제도 면제 요건 구체화 (6개월 전 서면 통지 + 사용 시기 서면 지정) |
| 예시 계산 | ✅ 3건 공식 일치 (10만×5, 8만×4, 12만×3) |
| 반올림 | ✅ 정수×정수 — 소수점 없음 |
| 테스트 케이스 | ✅ 87케이스 ALL PASS (기존 회귀 포함, AL 전용 unit 추가 예정) |
...
| 연말정산 (연말정산_환급액_계산기) | ✅ Verified | 2026-07-21 |
...
6. ~~연말정산 (연말정산_환급액_계산기)~~ ✅ Verified (2026-07-21, Phase 2 완료)

완료: 7종 전체 Verified 달성 (2026-07-21)

---

## 연말정산 계산기 검증 결과 (2026-07-21, Phase 2 완료) ✅ Verified

> 귀속연도: 2025년 / 법령: 소득세법 제47조·제55조·제59조

| 항목 | 결과 |
|---|---|
| 법령 확정 게이트 | ✅ PASS (소득세법 제47·55·59조 nts.go.kr 확인) |
| 계산 엔진 | ✅ modules/income_tax_calculator.py (11단계 Python mirror) |
| 4대보험 자동계산 | ✅ four-insurances 요율 상수 재사용 (A안) |
| 법령 직접 계산 오차 | ✅ 3케이스 전체 0원 (3천만/5천만/1억) |
| SP-8 변수명 노출 | ✅ 0건 (article_content + FAQ 전체) |
| 단위 테스트 | ✅ 37개 ALL PASS |
| 전체 회귀 | ✅ 178개 ALL PASS |
| content_caveat | crude_estimate → null (완전 구현으로 격상) |

**검증 세부 (3케이스, 1인·추가공제없음·기납부0)**

| 총급여 | ⑦산출세액 | ⑧세액공제 | ⑨결정세액 | 법령 오차 |
|---:|---:|---:|---:|:---:|
| 3,000만원 | 1,129,300 | 621,115 | 508,185 | 0원 |
| 5,000만원 | 3,472,167 | 660,000 | 2,812,167 | 0원 |
| 1억원 | 12,363,304 | 500,000 | 11,863,304 | 0원 |

**v1.1 이월 항목**: 자녀세액공제(소득세법 제59조의2), 특별세액공제(의료비·교육비·기부금), 신용카드, 주택자금
