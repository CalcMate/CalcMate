...
## 육아휴직급여 계산기 검증 결과 (2026-07-19, 진단 완료)
...
| 항목 | 결과 |
|------|------|
| 계산식 | ❌ **PL-1 Critical**: `gov_pct + company_pct` 구조 — 법령 무관, 전면 재설계 필요 |
| 법령 | ✅ 제70조 인용 OK, 금지 조항(제40조·근로기준법 제74조) 0건. ❌ 시행령 제95조(상한/하한) 미반영 |
| 경계값 | ❌ **PL-12**: 피보험단위기간 180일 처리 없음. 상한/하한 경계 없음 |
| 예외 처리 | ❌ **PL-9**: 음수/0 → null 미처리 |
| UI | ❌ **PL-10/11**: notices 없음, _formula 미설정 |
| 콘텐츠 | ❌ **PL-3**: 자녀 연령 "출산 후 1년" 오류. **PL-4**: 180일 미언급. **PL-5**: 육아휴직 권리 오류 |
| SP-8 재발 | ❌ **PL-6**: 코드 변수명 4종 FAQ 노출 |
| 특례 | ❌ **PL-8**: 6+6 부모 육아휴직 특례(2024년 1월 시행) 미구현 |
| 원칙-예외 (C-13) | ❌ **PL-15**: 3곳 모두 미준수 |
| 테스트 케이스 | ❌ 미작성 |

설계 범위: **재설계 필요** — 현재 입력 구조(`gov_pct`, `company_pct`) 자체가 법령과 무관.
권고 설계: 통상임금 입력 → 일반(80%, 상한150만, 하한70만) / 6+6 특례(100%, 월별 상한) 분기.

**육아휴직 현재 상태 (2026-07-19)**
Critical 6건 / Major 7건 / Minor 2건 — Phase 1(콘텐츠 오류) → Phase 2(계산식 전면 재설계) 순서
...
| 육아휴직 (육아휴직_급여_계산기) | 🔴 진단 완료 / Phase 1 대기 | — |
