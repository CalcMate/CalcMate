...
### R4. G4 계산 예시 게이트 — 연말정산 만원 단위·두 번째 예시 오판정 (2026-07-18 해결)
- **원인**: `_count_examples` 수치 패턴(`[\d,]+\s*원`)이 순수 숫자 형식("= 80,000원")만 인식하고
  한국어 단위형("= 50만원", "= 300만원")을 카운트 0으로 집계. 마커 패턴도 두 번째 예시 표현
  ("또 다른 예로", "두 번째 예시")을 미인식. 연말정산은 세금 금액이 만원 단위가 자연스러워
  writer가 정확히 2개 예시를 생성해도 G4 count=1로 판정되어 major HOLD 반복 발생.
- **해결**: "품질 기준 완화"가 아니라 **게이트 판정 정확도 개선**.
  수치 패턴에 `(?:만|억|천)?` 단위 선택자 추가, 마커 패턴에 `또\s*다른\s*예[시로]`·`두\s*번째\s*예시` 추가.
  오탐 방지: `예[시로]` 필수 후치로 예외/예방/예산 등 비예시 단어 구분. 수치 패턴은 `=` 앞치·
  `×` 뒤치 제약이 그대로 유지되어 일반 금액 나열과 계산 결과를 구분.
- **검증**: false negative 3건·false positive 5건·회귀 6건 = 총 14케이스 ALL PASS.
  실제 writer 출력 2건 확인 → G4 PASS 전환. 기존 6개 계산기 통과 유지.
