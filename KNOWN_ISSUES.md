...
## 4. 연말정산 — ✅ v1 완전 구현 (2026-07-21)
- **원인**: (해소) 누진세율·항목별 세액공제 미반영 단순 근사였음.
- **해소**: 소득세법 제47조(근로소득공제)·제55조(누진세율)·제59조(근로소득세액공제)·4대보험 자동계산 완전 구현.
  `modules/income_tax_calculator.py` 신규, `app_generator._compute_js()` 연말정산 분기 추가.
  37 테스트 ALL PASS, content_caveat crude_estimate 해제.
- **잔여(v1.1 이월)**: 자녀세액공제(소득세법 제59조의2), 특별세액공제(의료비·교육비·기부금), 신용카드, 주택자금.
...
- 법령 역할 분리: 남녀고용평등과 일·가정 양립 지원에 관한 법률 제19조(사용 권리) + 고용보험법 제70조(급여 요건) 병기

### PL-4 [Critical] ✅ Phase 1 해결 — FAQ[1] 수급 요건 오류 + C-13 원칙-예외
- 피보험단위기간 **180일** 명시, C-13 원칙-예외 구조 적용
- "최저임금 미충족 = 수급 제외" → 하한액 적용으로 정정. 3곳 일관성 PASS.

### PL-5 [Critical] ✅ Phase 1 해결 — FAQ[6] 육아휴직 권리 오류
