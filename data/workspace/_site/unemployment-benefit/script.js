...
window.computeResult = function(inputs){
  ...
  var age50p = [{lo:0,hi:12,d:120},{lo:12,hi:36,d:180},{lo:36,hi:60,d:210},{lo:60,hi:120,d:240},{lo:120,hi:Infinity,d:270}];
...
window.SM_CTA_RULES = {"default": {"text": "실업급여 더 알아보기", "links": [{"label": "신청 가이드 보기", "href": "/blog/unemployment-benefit-guide/"}, {"label": "전체 계산기", "href": "/"}], "analytics": "cta_ub_default"}, "rules": [{"condition": "outputs.total_benefit > 0", "text": "실업급여를 받을 수 있습니다", "links": [{"label": "신청 절차 안내", "href": "/blog/unemployment-benefit-guide/"}, {"label": "퇴직금도 확인하기", "href": "../severance-pay/"}], "analytics": "cta_ub_eligible"}, {"condition": "outputs.total_benefit === 0", "text": "피보험단위기간 6개월 이상이어야 합니다", "links": [{"label": "수급 조건 자세히 보기", "href": "/blog/unemployment-benefit-guide/"}, {"label": "전체 계산기", "href": "/"}], "analytics": "cta_ub_ineligible"}]};
window.SM_DYNAMIC_FAQ = [{"priority": 1, "condition": "outputs.total_benefit === 0 && (outputs.notices||[]).length > 0", "q": "피보험단위기간 6개월 미만이라 실업급여를 받을 수 없나요?", "a": "고용보험법 제40조에 따라 이직 전 18개월 이내에 피보험단위기간이 합산 180일(약 6개월) 이상이어야 구직급여를 수급할 수 있습니다. 복수의 직장에서 근무한 기간을 합산할 수 있습니다."}, {"priority": 2, "condition": "(outputs.notices||[]).some(function(n){return n.indexOf('상한')>=0 || n.indexOf('하한')>=0;})", "q": "구직급여 상한액/하한액이 적용됐는데 왜 그런가요?", "a": "구직급여는 1일 상한액(68,100원)과 하한액(최저임금의 80%)이 법으로 정해져 있습니다(고용보험법 제46조). 실제 임금과 무관하게 이 범위 내에서 지급됩니다."}, {"priority": 3, "condition": "outputs.total_benefit > 0", "q": "실업급여는 언제부터 받을 수 있나요?", "a": "퇴직 후 고용센터에 수급자격 신청(워크넷 구직 등록 후)을 하면 약 7~14일의 처리 기간 이후 지급이 시작됩니다. 자발적 퇴사는 원칙적으로 실업급여 수급이 불가합니다."}];
