...
window.computeResult = function(inputs){
  var weekly_hours = inputs["weekly_hours"] || 0;
  var hourly_wage = inputs["hourly_wage"] || 0;
  var out = {};
  out.notices = [];
  if (hourly_wage <= 0 || weekly_hours <= 0) { return null; }
  if (weekly_hours < 15) {
    out["weekly_holiday_pay"] = 0;
    out.notices.push("주 15시간 미만 근무 시 주휴수당이 발생하지 않습니다 (근로기준법 제18조제3항).");
...
window.SM_CTA_RULES = {"default": {"text": "주휴수당 더 알아보기", "links": [{"label": "주휴수당 완벽 가이드", "href": "/blog/weekly-holiday-allowance-guide/"}, {"label": "전체 계산기", "href": "/"}], "analytics": "cta_wha_default"}, "rules": [{"condition": "outputs.weekly_holiday_pay > 0", "text": "주휴수당이 발생합니다!", "links": [{"label": "주휴수당 가이드 보기", "href": "/blog/weekly-holiday-allowance-guide/"}, {"label": "퇴직금도 계산해 보기", "href": "../severance-pay/"}], "analytics": "cta_wha_eligible"}, {"condition": "outputs.weekly_holiday_pay === 0", "text": "주 15시간 이상 근무 시 주휴수당이 발생합니다", "links": [{"label": "조건 자세히 보기", "href": "/blog/weekly-holiday-allowance-guide/"}, {"label": "전체 계산기", "href": "/"}], "analytics": "cta_wha_ineligible"}]};
window.SM_DYNAMIC_FAQ = [{"priority": 1, "condition": "outputs.weekly_holiday_pay === 0 && (outputs.notices||[]).length > 0", "q": "주 15시간 미만인데 주휴수당이 0원인 이유는?", "a": "주휴수당은 1주 소정 근로시간이 15시간 이상인 근로자에게만 발생합니다 (근로기준법 제18조제3항). 주 15시간 미만 근로자는 주휴수당 지급 의무가 없습니다."}, {"priority": 3, "condition": "outputs.weekly_holiday_pay > 0", "q": "계산된 주휴수당이 실제 지급액과 다를 수 있나요?", "a": "본 계산기는 법정 기준으로 산출한 예상치입니다. 실제 지급액은 근로계약서 내용 및 사업장 규정에 따라 달라질 수 있습니다."}];
